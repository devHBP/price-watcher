<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Ajouter un URL Complementaire') }}
        </h2>
    </x-slot>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <!-- Formulaire -->
                    <form method="POST" action="{{ route('categories-url-concurrents.store') }}">
                        @csrf
                        <!-- Nom -->
                        <div class="mb-4">
                            ...
                            <input id="nom" type="text" name="nom" value="{{ old('nom') }}" placeholder="ex: Raquette - Padel XP" required
                        ...
                        <div class="mb-4">
                            <label class="block text-gray-700 dark:text-gray-200 text-sm font-bold mb-2" for="url_complement">
                                Complément d'url
                            </label>
                            <input id="url_complement" type="text" name="url_complement" value="{{ old('url_complement') }}" placeholder="ex: products/" required
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 dark:text-gray-200 leading-tight focus:outline-none focus:shadow-outline">
                ...
                <table class="min-w-full bg-white dark:bg-gray-800 shadow-md rounded-lg">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Complément URL</th>
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach ($categoriesUrlConcurrents as $url)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">{{ $url->id }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $url->nom }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $url->url_complement }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $url->concurrent->nom }}</td>
                                <td class="px-6 py-4">
                                    <a href="{{ route('categories-url-concurrents.edit', $url->id) }}">
                                ...
                                <td class="px-6 py-4">
                                    <form action="{{ route('categories-url-concurrents.delete', $url->id)}}" method="POST" onsubmit="return confirm('Voulez-vous vraiment supprimer cet URL ?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit">
