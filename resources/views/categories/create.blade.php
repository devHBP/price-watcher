                ...
                <table class="min-w-full bg-white dark:bg-gray-800 shadow-md rounded-lg">
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach ($categories as $categorie)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">{{ $categorie->id }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $categorie->nom }}</td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <form action="{{ route('categories.delete', $categorie->id)}}" method="POST" onsubmit="return confirm('Voulez-vous vraiment supprimer ce produit ?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit">
