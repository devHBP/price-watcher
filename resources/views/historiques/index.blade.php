    ...
    <div class="overflow-x-auto mt-3.5">
        <table class="min-w-full bg-white dark:bg-gray-800 shadow-md rounded-lg">
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                @foreach ($historiques as $prix)
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">{{ $prix->id }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $prix->prix }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $prix->produitConcurrent->designation_concurrent }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $prix->created_at }}</td>
                        <td class="px-6 py-4">
                            <a href="">
                        ...
                        <td class="px-6 py-4">
                            <form action="" method="POST" onsubmit="return confirm('Voulez-vous vraiment supprimer ce produit ?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit">
